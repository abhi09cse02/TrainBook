package com.practise.trainbook.service.impl;
...
import com.practise.trainbook.beans.TrainBean;
import com.practise.trainbook.beans.TrainException;
import com.practise.trainbook.constant.ResponseCode;
import com.practise.trainbook.service.TrainService;
import com.practise.trainbook.utility.DBUtil;
...
public class TrainServiceImpl implements TrainService {
	...
	public String addTrain(TrainBean train) {
		...
		String query = "INSERT INTO RESERVATION.TRAIN VALUES(?,?,?,?,?,?)";
		try {
	...
	public String deleteTrainById(String trainNo) {
		String responseCode = ResponseCode.FAILURE.toString();
		String query = "DELETE FROM RESERVATION.TRAIN WHERE TR_NO=?";
		try {
			Connection con = DBUtil.getConnection();
			PreparedStatement ps = con.prepareStatement(query);
	...
	public String updateTrain(TrainBean train) {
		String responseCode = ResponseCode.FAILURE.toString();
		String query = "UPDATE RESERVATION.TRAIN SET TR_NAME=?, FROM_STN=?,TO_STN=?,SEATS=?,FARE=? WHERE TR_NO=?";
		try {
			Connection con = DBUtil.getConnection();
			PreparedStatement ps = con.prepareStatement(query);
	...
	public TrainBean getTrainById(String trainNo) throws TrainException {
		...
		String query = "SELECT * FROM RESERVATION.TRAIN WHERE TR_NO=?";
		try {
			Connection con = DBUtil.getConnection();
			PreparedStatement ps = con.prepareStatement(query);
			ps.setString(1, trainNo);
	...
	public List<TrainBean> getAllTrains() throws TrainException {
		...
		String query = "SELECT * FROM RESERVATION.TRAIN";
		try {
			Connection con = DBUtil.getConnection();
			PreparedStatement ps = con.prepareStatement(query);
			ResultSet rs = ps.executeQuery();
	...
	public List<TrainBean> getTrainsBetweenStations(String fromStation, String toStation) throws TrainException {
		List<TrainBean> trains = null;
		String query = "SELECT * FROM RESERVATION.TRAIN WHERE UPPER(FROM_STN) LIKE UPPER(?) AND UPPER(TO_STN) LIKE UPPER(?)";
